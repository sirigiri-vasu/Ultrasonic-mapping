...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown():
		...
		if line == '':
			...
			if str(line).find('$GPGGA') > 0 and len(line_parsed[2])>0 and len(line_parsed[3])>0 and len(line_parsed[4]) >0 and len(line_parsed[5])>0 and len(line_parsed[6])>0:
				#rospy.loginfo(line)
				line_parsed = str(line).split(",")
				...
				rospy.loginfo("not valid GPGGA msg")
